<template>
  <div>
    <el-steps :active="2" align-center>
      <el-step title="Select Movie Session"></el-step>
      <el-step title="Select Seats"></el-step>
      <el-step title="Pay Within 15 Minutes"></el-step>
      <el-step title="Pick Up Tickets at Cinema"></el-step>
    </el-steps>
    <div class="whole">
      <div class="left">
        <div class="seat-example">
          <span class="seat" :class="isSelected[0]"></span>
          <span style="margin: 0 20px">Available Seats</span>
          <span class="seat" :class="isSelected[3]"></span>
          <span style="margin: 0 20px">Sold Seats</span>
          <span class="seat" :class="isSelected[2]"></span>
          <span style="margin: 0 20px">Selected Seats</span>
        </div>

        <div class="screen">Center Screen</div>

        <div class="seat-content">
          <div class="seat-row" v-for="(value, key) in seats">
            <div>
              <span class="seat-row-label">{{key}}</span>
            </div>
            <div style="display: flex; justify-content: flex-end">
                <span class="seat"
                      :class="isSelected[seats[key][index]]"
                      @click="pressSeat(key, index)"
                      v-for="(item, index) in value"></span>
            </div>
            <div>
              <span class="seat-row-label">{{key}}</span>
            </div>
          </div>
        </div>
      </div>

      <div class="right">
        <div class="right-content">
          <div class="right-header">
<!--            <div class="poster">-->
<!--              <img :src="session.sysMovie.moviePoster" alt="">-->
<!--            </div>-->
            <div class="movie-info">
              <p style="font-size: 20px;font-weight: 700">{{session.sysMovie.movieName}}</p>
              <span>Genres: {{session.sysMovie.movieCategoryList.join('/')}}</span>
              <span>Duration: {{session.sysMovie.movieLength}} minutes</span>
            </div>
          </div>
          <div class="right-content">
            <div class="info-item">
              <span>Cinema: </span>
              <span>{{session.sysHall.sysCinema.cinemaName}}</span>
            </div>
            <div class="info-item">
              <span>Hall: </span>
              <span>{{session.sysHall.hallName}}</span>
            </div>
            <div class="info-item">
              <span>Language: </span>
              <span>{{session.languageVersion}}</span>
            </div>
            <div class="info-item">
              <span>Session: </span>
              <span>{{session.sessionDate}} {{session.playTime}}</span>
            </div>
            <div class="info-item">
              <span>Price: </span>
              <span>${{session.sessionPrice}}/ticket</span>
            </div>
            <span style="color: #999">Seats: </span>
            <div class="seat-chose">
              <span class="ticket" v-if="pickedSeats.length > 0" v-for="item in pickedSeats">{{ item }}</span>
            </div>
            <div class="info-item" style="align-items: center">
              <span style="color: #333">Total:</span>
              <span style="font-size: 20px;color: #f03d37">${{session.sessionPrice * pickedSeats.length}}</span>
            </div>
            <div style="text-align: center;margin-top: 30px">
              <el-button type="danger" round @click="submitBill">Submit Order</el-button>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import moment from 'moment'
import { Message } from "element-ui";
export default {
  name: "ChooseSeat",
  data() {
    return {
      isSelected: {
        0: 'seat-default',
        1: 'seat-no',
        2: 'seat-pick',
        3: 'seat-sold'
      },
      seats: {},
      sessionId: this.$route.params.sessionId,
      session: {
        sysMovie: {
          movieCategoryList: []
        },
        sysCinema: {},
        sysHall: {}
      },
      pickedSeats: [],
      addForm: {
        userId: 0,
        sessionId: 0,
        seats: ''
      }
    }
  },
  created() {
    this.getSession()
  },
  methods: {
    async getSession(){
      const {data: resp} = await axios.get('sysSession/find/' + this.sessionId )
      if(resp.code !== 200) return this.$message.error(resp.msg)
      this.session = resp.data
      this.session.sysMovie.moviePoster = this.global.base + JSON.parse(this.session.sysMovie.moviePoster)[0]
      this.seats = JSON.parse(resp.data.sessionSeats)
      this.session.sysMovie.movieCategoryList = this.session.sysMovie.movieCategoryList.map((obj, index) => {
        return obj.movieCategoryName
      })
      console.log('Seat status when loading page')
      console.log(this.seats)
    },
    pressSeat(key, idx) {
      let seat_str = key + "-" + (idx+1)
      // Selecting a seat
      if (this.seats[key][idx] === 0) {
        // Check if more than 5 seats are selected, if so, can't select
        if (this.pickedSeats.length === 5) {
          this.$alert('You can only select up to five seats', 'Notice', {
            confirmButtonText: 'OK',
            type: 'warning'
          }).catch(err => err)
          return
        }
        this.$set(this.seats[key], idx, 2)
        this.pickedSeats.push(seat_str)
      } else if (this.seats[key][idx] === 2) {
        // Deselecting a seat
        this.$set(this.seats[key], idx, 0)
        this.pickedSeats.splice(this.pickedSeats.indexOf(seat_str), 1)
      }
      console.log(this.pickedSeats)
    },
    async submitBill() {
      // Get token, check login status
      const token = window.sessionStorage.getItem("token")
      if (!token) {
        window.sessionStorage.setItem('sessionId', this.session.sessionId)
        await this.$alert('Sorry! Please login before submitting the order', 'Exception Notification for Submitting Order', {
          confirmButtonText: 'OK',
          callback: action => {
            this.$router.push('/login')
          }
        })
        return
      }
      // Check if seats are selected, if not, warn the user
      if (this.pickedSeats.length === 0){
        await this.$alert('Sorry! You have not selected any seats yet, please select seats before submitting the order.', 'Exception Notification for Submitting Order', {
          confirmButtonText: 'OK',
          callback: action => {
            this.$router.push('/chooseSeat/' + this.sessionId)
          }
        })
        return
      }
      // Get session seat information
      const { data : curSession } = await axios.get('sysSession/find/' + this.sessionId)
      let sessionSeats = JSON.parse(curSession.data.sessionSeats)
      // Parse selected seats from the order, update seat information
      for (let seat of this.pickedSeats) {
        let [row, col] = seat.split('-')
        if (sessionSeats[row][col - 1] === 3){
          // Update seat information
          this.seats = JSON.parse(curSession.data.sessionSeats)
          await this.$alert('Sorry! The seat ' + seat + ' you selected is already occupied. Please choose again.', 'Exception Notification for Seat Selection', {
            confirmButtonText: 'OK',
            callback: action => {
              this.$router.push('/chooseSeat/' + this.sessionId)
            }
          })
          return
        } else {
          sessionSeats[row][col - 1] = 3
        }
      }
      console.log('Seat status after submitting order')
      console.log(sessionSeats)

      this.addForm.userId = JSON.parse(window.sessionStorage.getItem('loginUser')).userId
      this.addForm.sessionId = this.sessionId
      this.addForm.seats = JSON.stringify(this.pickedSeats)
      axios.defaults.headers.post['Content-Type'] = 'application/json'
      const { data: res } = await axios.post('sysBill', JSON.stringify({sysBill: this.addForm, sessionSeats: JSON.stringify(sessionSeats)}))
      // const { data: res} = await axios.post('sysBill', JSON.stringify(this.addForm));
      if(res.code !== 200) return this.$message.error('Failed to add order!')
      await this.$router.push('/billDetail/' + res.data.billId)
    }
  }
}
</script>

<style scoped>

.el-step {
  margin: 40px auto;
}

.whole{
  width: 1200px;
  margin: 0 auto;
  border: 1px solid #e5e5e5;
  display: flex;
}

.left {
  width: 820px;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  border-right: 1px solid #e5e5e5;
}

.right{
  width: 340px;
  background-color: #f9f9f9;
  padding: 20px;
}

.seat {
  display: inline-block;
  width: 30px;
  height: 26px;
  margin: 0 5px;
  background-position: 0 1px;
}

.seat-default {
  background: url("../../assets/sofa-unpick.png") no-repeat;
  background-size: cover;
}

.seat-empty {
  background: url("../../assets/sofa-empty.png") no-repeat;
  background-size: cover;
}

.seat-sold {
  background: url("../../assets/sofa-forbidden.png") no-repeat;
  background-size: cover;
}

.seat-pick{
  background: url("../../assets/sofa-pick.png") no-repeat;
  background-size: cover;
}

.seat-example{
  display: flex;
  align-items: center;
  margin: 20px 0 0 0;
}

.screen{
  width: 550px;
  padding-top: 50px;
  text-align: center;
  font-size: 16px;
  color: #666;
  background: url(data:image/png;base64,) no-repeat;
  background-position-x: center;
  position: relative;
  margin: 40px 0 40px 0;
}

.seat-row {
  white-space: nowrap;
  margin: 10px 30px;
  padding: 0 10px;
  text-align: center;
  display: flex;
  justify-content: space-between;
}

.seat-row-label {
  font-size: 18px;
}


.seat-content{
  margin-bottom: 40px;
  overflow-x: scroll;
  width: 90%;
  padding: 10px;
}

.poster{
  width: 115px;
  height: 158px;
}

.poster>img{
  width: auto;
  height: 100%;
}

.right-header{
  display: flex;
}

.movie-info {
  display: flex;
  flex-direction: column;
  //align-items: center;
  //margin-left: 20px;
}

.info-item{
  display: flex;
  margin: 15px 0;
  font-size: 14px;
}

.info-item :first-child{
  color: #999999;
}

.seat-chose{
  margin-top: 20px;
  margin-left: 42px;
  margin-bottom: 20px;
  position: relative;
  top: -5px;
}

.ticket{
  cursor: default;
  position: relative;
  font-size: 13px;
  color: #f03d37;
  display: inline-block;
  width: 60px;
  height: 30px;
  line-height: 30px;
  text-align: center;
  margin: 0 12px 10px 0;
  background: url(data:image/png;base64,) no-repeat;
}
</style>
