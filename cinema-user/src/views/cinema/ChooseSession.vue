      ...
      <div class="header-inner clearfix">
        ...
        <div class="movie-info-right">
          <div class="movie-info-msg">
            ...
            <div class="phone-number">Phone: {{cinemaInfo.cinemaPhone}}</div>
            <div style="margin: 25px 0;">Cinema Services ————————————————————</div>
            <div class="service"><span class="service-tag">Free Parking</span> Free parking 1 hour before and 1 hour after the movie</div>
            <div class="service"><span class="service-tag">Free Happy Water</span> During holidays, each ticket can receive a bottle of Happy Water</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="movie-info-detail clearfix">
        <div class="main-content">
          <div class="crumbs-nav-container">
            <a href="/welcome">Cinema</a> &gt; <a href="/aboutUs"> {{cinemaInfo.cinemaName}}</a>
          </div>
          <!-- All movies currently showing in theaters -->
          ...
          <!-- Currently selected movie information -->
          <div class="show-list"
               v-if="cinemaInfo.sysMovieList !== null
            && cinemaInfo.sysMovieList.length !== 0">
            <div class="show-movie-info">
              ...
              <div class="show-movie-desc">
                <div>
                  ...
                  <span>{{movieDict[activeMovie].movieLength}} minutes</span>
                </div>
                <div>
            ...
            <div class="show-date" v-if="sessions !== null && sessions.length !== 0">
              ...
              <span class="date-item"
                    ...
                    :key="key">{{calculateDate(key)}}</span>
            </div>
            <div class="session-info">
              <el-table :data="sessionDict[activeDate]" stripe>
                <el-table-column
                  label="Screening Time">
                  <template slot-scope="scope">
                    <span class="play-time">{{scope.row.playTime}}</span>-<span class="play-time">{{scope.row.endTime}}</span>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column label="Hall Tips">
                  <template slot-scope="scope">
                    {{scope.row.sessionTips === null ? 'None' : scope.row.sessionTips}}
                  </template>
                </el-table-column>
                ...
                <el-table-column label="Select Seat">
                  <template slot-scope="scope">
                    <el-button type="primary" round @click="toChooseSeat(scope.row.sessionId)">Select Seat</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      // Used to obtain movie information with specified id
      movieDict: { 0: {
          movieBoxOffice: 0,
          movieCategoryList: [],
          movieId: 0,
  ...
  methods: {
    ...
    async getNewSessionList() {
      ...
      if (res.code !== 200) return this.$message.error('Failed to get information')
      this.sessions = res.data.sessions
      // Group sessions by date
      this.sessionDict = {}
      for (let session of this.sessions) {
